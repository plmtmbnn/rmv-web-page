...
import { newsData } from "./news-data-list";
import { useState, useMemo } from "react";
import { useSearchParams, useRouter } from "next/navigation";

const categories = ["Semua", "Literasi Keuangan", "Inklusi Keuangan"];
...
export default function NewsListPage() {
	...
	const updateURL = (category: string, search: string) => {
		...
		router.push(
			queryString
				? `/investor/financial-literacy-and-inclusion?${queryString}`
				: "/investor/financial-literacy-and-inclusion",
			{ scroll: false },
		);
	};
	...
	const handleReset = () => {
		...
		setSearchQuery("");
		setVisibleItems(ITEMS_PER_PAGE);
		router.push("/investor/financial-literacy-and-inclusion");
	};
			...
			<div className="container mx-auto px-6 md:px-12 lg:px-20 py-16 md:py-20">
				...
				<div className="mb-12 pb-8 border-b border-slate-200">
					...
					<p className="text-slate-600 text-lg max-w-2xl">
						Temukan beragam artikel pilihan seputar Literasi dan Inklusi
						Keuangan melalui menu di bawah ini untuk memperluas wawasan Anda.
					</p>
				...
				{displayedNews.length > 0 &&
					...
					!searchQuery && (
						<div className="mb-12">
							<Link
								href={`/investor/financial-literacy-and-inclusion/${newsData[0].slug}`}
								...
								<div className="grid grid-cols-1 lg:grid-cols-2">
									...
									<div className="p-8 lg:p-10 flex flex-col justify-center">
										<div className="flex items-center gap-4 mb-4">
											...
											<span className="flex items-center gap-2 text-sm text-slate-500"></span>
										</div>
						...
						<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
							{displayedNews
								...
								.map((news, index) => (
									<Link
										key={news.slug}
										href={`/investor/financial-literacy-and-inclusion/${news.slug}`}
