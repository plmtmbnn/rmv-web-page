 ...
 */
function ImageTile({ src, alt = "", className = "", priority = false }: TileProps) {
  return (
    <div className={`group relative overflow-hidden rounded-2xl bg-slate-200 shadow-sm transition-all duration-500 hover:shadow-xl ${className}`}>
      <Image
        src={src}
        alt={alt}
        fill
 ...
 */
function ContactItem({ icon: Icon, title, content, actionLabel, type, href, colorClass }: ContactCardProps) {
  ...
  const baseColors: Record<string, string> = {
    ...
    green: "bg-emerald-50 text-emerald-600 group-hover:bg-emerald-600 group-hover:text-white border-emerald-100",
    red: "bg-red-50 text-red-600 group-hover:bg-red-600 group-hover:text-white border-red-100",
  };

  const Wrapper = type === 'link' ? 'a' : 'div';
  const wrapperProps = type === 'link' ? { href, target: "_blank", rel: "noopener noreferrer" } : { onClick: handleCopy };
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between">
        <div className={`rounded-xl p-3 transition-colors duration-300 ${baseColors[colorClass]}`}>
      ...
      <div>
        ...
        <p className={`mt-1 text-lg font-medium text-slate-900 ${type === 'link' ? 'break-all' : 'break-all'}`}>
          {content}
        </p>
      </div>

      <div className="mt-auto pt-2">
        <span className={`text-xs font-bold underline decoration-2 underline-offset-4 opacity-0 transition-opacity duration-300 group-hover:opacity-100 text-${colorClass === 'green' ? 'emerald' : colorClass}-600`}>
            ...
            <article className="relative h-full overflow-hidden rounded-[2.5rem] bg-white p-8 shadow-xl ring-1 ring-slate-900/5 md:p-12">
              ...
              <div className="relative z-10">
                <div className="flex items-center gap-3 mb-8">
                    ...
                    <span className="text-sm font-bold tracking-widest text-[#d94e35] uppercase">Profil Perusahaan</span>
                </div>

                <h3 className="font-serif text-3xl text-slate-900 mb-6 leading-tight">
                  Sekilas Tentang <br />
                  <span className="font-bold text-[#0b203f]">PT. Reliance Modal Ventura</span>
                </h3>

                <div className="space-y-6 text-slate-600 md:text-lg leading-relaxed">
                  <p>
                    Didirikan berdasarkan akta Nomor 116 tanggal 6 Juni 2015 dihadapan Rosita Rianauli Sianipar, SH, M.Kn, Notaris di Jakarta.
                  </p>
                  <p>
                    Kami telah memperoleh persetujuan resmi dari Kementerian Hukum dan Hak Asasi Manusia (Nomor AHU-2443277.AH.01.01.Tahun 2015) dan beroperasi dengan standar tata kelola perusahaan yang ketat.
                  </p>
                </div>
              </div>
            </article>
             ...
             <div className="grid grid-cols-2 grid-rows-3 gap-4 h-full">
                ...
                <div className="col-span-1 row-span-1 relative rounded-2xl bg-slate-900 flex items-center justify-center overflow-hidden group">
                     ...
                     <div className="relative z-10 text-center">
                        <span className="block text-3xl font-bold text-white">Reliance</span>
                        <span className="text-xs text-white/80 tracking-widest uppercase">Modal Ventura</span>
                     </div>
                </div>
             </div>
          </div>
        ...
        <div className="relative">
            <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-4">
                <div>
                    ...
                    <p className="text-slate-600 mt-2">Informasi kontak resmi perusahaan</p>
                </div>
            </div>
